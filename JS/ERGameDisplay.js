...
function processKillerKilleeEvent(event, usedPlayers) {
    ...
    // Killer 선택
    killers = event.killer.map(() =>
        getRandomWeightedPlayer(
            gameState.players.filter((player) => !usedPlayers.has(player.id)),
            usedPlayers,
            eventType
        )
    );
    ...
    killees = event.killee.map(() => {
        ...
        do {
            killee = getRandomWeightedPlayer(
                ...
                usedPlayers,
                eventType,
                killers[0]?.id // 같은 이벤트의 Killer 제외
            );
            retries++;
        } while (retries < 3 && (!killee || killers.some((k) => k.id === killee.id)));

        return killee;
    });

    // 선택된 플레이어를 usedPlayers에 추가
    killers.forEach((killer) => {
        if (killer) usedPlayers.add(killer.id);
    });
    killees.forEach((killee) => {
        if (killee) usedPlayers.add(killee.id);
    });
...
function processBenefitEvent(event, usedPlayers) {
    console.log("Processing BenefitTarget Event:", event);

    // 텍스트에 있는 모든 `cn` 패턴 추출
    const idPattern = /c(\d+)/g; // c<숫자> 패턴
    const matches = Array.from(event.textEvent.matchAll(idPattern));
    if (matches.length === 0) {
        console.warn("No valid IDs found in event text.");
        return;
    }

    console.log("Found IDs in Event Text:", matches.map((match) => match[0]));

    let eventText = event.textEvent;
    const benefitTargetsArray = []; // 여러 benefitTarget 저장 배열

    // 각 ID에 대해 처리
    matches.forEach((match) => {
        const targetId = match[1]; // 매칭된 ID
        console.log(`Processing ID: ${targetId}`);

        // 사용되지 않은 플레이어 중 선택
        const benefitTargets = gameState.players.filter(
            (player) => player.isAlive && !usedPlayers.has(player.id)
        );

        if (benefitTargets.length === 0) {
            console.warn("No valid benefit targets available for ID:", targetId);
            return;
        }

        // 무작위로 플레이어 선택
        const randomIndex = Math.floor(Math.random() * benefitTargets.length);
        const benefitTarget = benefitTargets[randomIndex];

        if (!benefitTarget) {
            console.warn("No valid benefitTarget found for ID:", targetId);
            return;
        }

        console.log("Selected BenefitTarget ID:", benefitTarget.id);
        console.log("Selected BenefitTarget Name:", benefitTarget.name);

        // 선택된 플레이어를 usedPlayers에 추가
        usedPlayers.add(benefitTarget.id);

        // 텍스트 내의 ID를 이름으로 대체
        eventText = eventText.replace(
            new RegExp(`c${targetId}`, "g"),
            benefitTarget.name
        );

        // 배열에 추가
        benefitTargetsArray.push({
            id: benefitTarget.id,
    ...
    // 유효 대상이 없는 경우 처리
    if (benefitTargetsArray.length === 0) {
        console.warn("No valid benefit targets found for this event.");
        return;
    }

    console.log("Final Updated Event Text:", eventText);
    console.log("Benefit Targets Array:", benefitTargetsArray);

    // 이벤트 로그 업데이트
    gameState.killerLogs.push({
        text: eventText,
        benefitTargets: benefitTargetsArray,
    });

    // 이벤트 렌더링
    renderEvent({
        text: eventText,
        images: benefitTargetsArray.map((target) => ({
            image: target.image,
            name: target.name,
